...
export function TagManagementDialog({ open, onOpenChange }: TagManagementDialogProps) {
  ...
  const seedTagsMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      toast({ 
        title: "Success", 
        description: `Successfully seeded ${data.count} preset tags` 
      });
    },
    onError: () => {
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-4xl max-h-[90vh]" data-testid="dialog-tag-management">
          <DialogHeader>
            ...
            <DialogDescription>
              Manage investigation tags and document coding labels ({tags.length} total: {tags.filter(t => t.isPreset).length} preset, {tags.filter(t => !t.isPreset).length} custom)
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex gap-2 justify-end border-b pb-4">
            ...
            <Button onClick={() => setIsCreateDialogOpen(true)} data-testid="button-create-tag">
          ...
          <div className="max-h-[60vh] overflow-y-auto">
            <Tabs value={selectedCategory} onValueChange={setSelectedCategory} className="w-full">
              <TabsList className="w-full justify-start">
                <TabsTrigger value="all" data-testid="tab-all-tags">All</TabsTrigger>
                <TabsTrigger value="investigation_type" data-testid="tab-investigation-type">Investigation</TabsTrigger>
                ...
                <TabsTrigger value="evidence_type" data-testid="tab-evidence-type">Evidence</TabsTrigger>
                <TabsTrigger value="custom" data-testid="tab-custom">Custom</TabsTrigger>
              </TabsList>

              <TabsContent value={selectedCategory} className="space-y-6 mt-6 px-1">
                {isLoading ? (
