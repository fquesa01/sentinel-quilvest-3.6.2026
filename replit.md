...
- **Court Pleadings System:** Upload and manage legal documents (complaints, motions, briefs, discovery) with automatic text extraction from PDF/Word/text files, RAG indexing for AI-powered search, and file persistence in object storage. Includes pleading type classification, filing date tracking, and download functionality. Documents are automatically indexed for Ambient Intelligence discovery. **Court Docket Tab**: Dedicated case detail tab (positioned between Overview and Evidence) displaying filings in authentic court docket spreadsheet format. Columns: Filing Date, Title, Filing Party (plaintiff/defendant/court/third_party with color-coded badges), Status (Court Filing/Draft). Features: inline editing of title, date, party, and status; auto-extraction of document headline from OCR text as title; add/delete filing dialog with file upload. Database enums: filing_party and filing_status. API: PATCH /api/court-pleadings/:id for inline updates.
- **Ambient Intelligence:** Real-time meeting transcription with AI-powered document discovery, smart search queries, and risk-based results. **Real-Time Meeting Insights**: Generates summary, key_point, and action_item suggestions every 20 seconds during transcription using Google Gemini. Insight cards display at the top of the AI Suggestions Panel in both "all" and "focused" modes with color-coded left borders (blue=summary, amber=key_point, green=action_item), appropriate icons (MessageCircle, Lightbulb, ListTodo), confidence indicators, and dismiss functionality. Frontend transforms insight suggestions separately from document-based suggestions via the InsightSuggestion interface.
